...
`chapters`、`output_dir`、`style`、`bibliography` のパスは、`book.yml` のあるディレクトリからの相対パスとして解決されます。
...
本の表紙ページ。本を開いたときに最初に表示される。TOC には表示されない。
...
| ` ```mermaid` | ダイアグラム（フローチャート、シーケンス図、棒・折れ線・円グラフ、ガント、マインドマップ等） | [mermaid](https://mermaid.js.org/) | MIT |
| ` ```math` | 数式（LaTeX 記法） | [KaTeX](https://katex.org/) | MIT |
| ` ```functionplot` | 関数グラフ | [function-plot](https://mauriciopoppe.github.io/function-plot/) + d3 | MIT |
...
- `y = <式>` — 通常の関数
- `r = <式>` — 極座標
- `parametric: <x式>, <y式>` — パラメトリック曲線
- 式のみ — `y =` 省略可

オプション（1行に1つ）:
- `range` / `xrange: [min, max]` — X軸の範囲（`pi` 使用可、例: `[-2pi, 2pi]`）
- `yrange: [min, max]` — Y軸の範囲
- `width: <px>` — 幅（デフォルト: 600）
- `height: <px>` — 高さ（デフォルト: 400）
- `title: <テキスト>` — タイトル
- `grid: true` — グリッド表示
...
Markdown のリンク記法を使って参考文献を引用する。引用された文献は本の末尾に「参考文献」セクションとして自動生成される。

#### 設定

`book.yml` に `bibliography` フィールドで参考文献データファイルを指定する。拡張子で形式を自動判定する。
...
- エントリタイプ（`@book`、`@article` 等）を保持し、表示フォーマットの切替に使用
- フィールド値は `{...}` または `"..."` で記述
- `{...}` ではネスト 1 段階に対応（例: `{The {Ruby} Language}`）
- `%` で始まる行はコメントとして無視

#### 対応フィールド

YAML・BibTeX 共通で以下のフィールドに対応:

| フィールド | 用途 | 表示例 |
|-----------|------|--------|
| `author` | 著者 | Matsumoto, Yukihiro |
| `title` | タイトル | *The Ruby Programming Language* |
| `year` | 出版年 | 2008 |
| `url` | URL | タイトルがリンクになる |
| `publisher` | 出版社 | O'Reilly |
| `journal` | 雑誌名 | *Communications of the ACM* |
| `booktitle` | 書籍/会議名 | *Proc. OOPSLA* |
| `volume` | 巻 | Vol. 12 |
| `number` | 号 | (3) |
| `pages` | ページ | pp. 45-67 |
| `edition` | 版 | 2nd ed. |
...
`url` があればタイトルがリンクになる。`doi` があれば末尾に DOI リンクを追加。

#### 記法

| 記法 | 表示 |
|------|------|
| `[Ruby](#cite:matz1995)` | Ruby<sup>[Yukihiro, 1995]</sup> |

#### 動作

- `<a href="#cite:KEY">` が kramdown により生成される
- ビルド時にリンクを検出し、`<span>` に置換。上付きで `[著者姓, 年]` のリンクを付加
- リンクは参考文献セクションの該当エントリへジャンプする
- hover で文献の完全な情報が表示される（全フィールドを含む）
- 参考文献セクションは著者名・年でソートされる
- `url` がある場合、参考文献セクションのタイトルがリンクになる
- `doi` がある場合、末尾に DOI リンクが追加される
- 存在しないキーを参照した場合、警告を出力し `[key?]` と赤色で表示する（ビルドは中断しない）
- `bibliography` 未設定の場合、参考文献セクションは生成されない
...
ligarb librarium             マルチブック図書館サーバーを起動（*/book.yml を自動検出）
...
```
ligarb build [CONFIG]       CONFIG のデフォルトは book.yml
```
...
2. **Claude レビュー**: コメントを送信すると Claude（Opus）が `book.yml` を読み、必要な章や参考文献ファイルを参照して改善提案を返す。提案には `<patch>` ブロック（具体的な差分）が含まれ、複数の章や参考文献ファイルにまたがるパッチも可能
...
マルチブックモードの図書館サーバーを起動する。カレントディレクトリの `*/book.yml` を自動検出し、すべての本をまとめて配信する。

```
ligarb librarium                マルチブック図書館サーバーを起動
ligarb librarium --port 8080    ポート指定（デフォルト: 3000）
```

`ligarb serve --multi */book.yml` と同等の動作。機能の詳細は `ligarb serve` のマルチブックモードを参照。

子ディレクトリに `book.yml` が 1 つも見つからない場合はエラーで終了する。
