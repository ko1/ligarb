...
`ligarb serve` の[マルチブックモード](06-serve.md)では、トップページの「+ Write a new book」ボタンからブラウザ上で AI 執筆を開始できます。
フォームにタイトルや対象読者を入力するだけで、`brief.yml` の作成から `ligarb write` の実行、ビルドまでバックグラウンドで自動実行されます。

執筆の進行状況はリアルタイムでステータスバッジに反映され、完了した本はそのまま閲覧できます。
詳細は[プレビューとレビュー](06-serve.md)の章を参照してください。

## ポイント

- **`ligarb write` を使う**: 企画書を書いて実行するだけで、本の生成からビルドまで自動化できます
- **ブラウザから使う**: マルチブックモードならブラウザ上のフォームから AI 執筆を開始できます
