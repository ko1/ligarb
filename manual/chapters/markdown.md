...
> [!TIP]
> ビルド時に Node.js が利用可能な場合、ligarb は各 mermaid ブロックを
> バンドル済みの mermaid パーサで検証します。構文エラーがあると
> `ファイル:行` 付きの警告が表示されます（ビルド自体は成功し、出力 HTML には
> エラーボックスが表示されます）。Node.js が無い環境ではチェックはスキップされます。
> なお、ノードラベル内の `<br>` などの HTML はチェックの対象外で、警告は出ません。
...
- 存在しないキーを `#cite:` で参照すると警告が出力され、`[key?]` と赤色で表示されます（ビルドは中断しません）
